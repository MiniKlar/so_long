...
/*   Updated: 2025/02/14 05:31:56 by lomont           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int argc, char **argv)
{
	...
	if (argc == 2)
	{
		...
		if (init_data->fd > 0)
		{
			...
			game_data->img = mlx_texture_to_image(all_struct->mlx, game_data->floor);
			//game_data->exit= mlx_load_png("./src/img/B.png");
			//game_data->img4 = mlx_texture_to_image(all_struct->mlx, game_data->exit);
			while (tab[x] != 0)
			{
				y = 0;
				while (tab[x][y] != '\0')
				{
					mlx_image_to_window(all_struct->mlx, game_data->img, y * SIZE, x * SIZE);
					y++;
				}
				x++;
			}
			...
			game_data->img2 = mlx_texture_to_image(all_struct->mlx, game_data->wall);
			while (tab[x] != 0)
			{
				y = 0;
				while (tab[x][y] != '\0')
				{
					if (tab[x][y] == '1')
						mlx_image_to_window(all_struct->mlx, game_data->img2, y * SIZE, x * SIZE);
					y++;
				}
				x++;
			}
			...
			game_data->img3 = mlx_texture_to_image(all_struct->mlx, game_data->perso_T);
			//game_data->C4= mlx_load_png("./src/img/C4.png");
			//game_data->img5 = mlx_texture_to_image(all_struct->mlx, game_data->C4);
			while (tab[x] != 0)
			{
				y = 0;
				while (tab[x][y] != '\0')
				{
					//if (tab[x][y] == 'P')
						//mlx_image_to_window(all_struct->mlx, game_data->img3, y * SIZE, x * SIZE);
					//else if (tab[x][y] == 'E')
						//mlx_image_to_window(all_struct->mlx, game_data->img4, y * SIZE, x * SIZE);
					if (tab[x][y] == 'P')
						mlx_image_to_window(all_struct->mlx, game_data->img3, y * SIZE, x * SIZE);
					y++;
				}
